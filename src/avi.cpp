...
int AVIFile::GetDVFrameInfo( off_t &offset, int &size, int frameNum )
{
	...
	case AVI_LARGE_INDEX:
		...
		if ( i != current_ix00 )
		{
			if( lseek( fd, indx[ 0 ] ->aIndex[ i ].qwOffset + RIFF_HEADERSIZE, SEEK_SET ) == ( off_t ) - 1 )
				return -1;
			if(! read( fd, ix[ 0 ], indx[ 0 ] ->aIndex[ i ].dwSize - RIFF_HEADERSIZE ) )
				return -1;
			current_ix00 = i;
		}
...
void AVIFile::PrintDirectoryEntryData( const RIFFDirEntry &entry ) const
{
	...
	if ( entry.type == make_fourcc( (char*) "avih" ) )
	{
		...
		if( lseek( fd, entry.offset, SEEK_SET ) == ( off_t ) - 1 )
			return;
		if(read( fd, &main_avi_header, sizeof( MainAVIHeader ) ) < 0)
			return;
	...
	else if ( entry.type == make_fourcc( (char*) "strh" ) )
	{
		...
		if( lseek( fd, entry.offset, SEEK_SET ) == ( off_t ) - 1 )
			return;
		if(read( fd, &avi_stream_header, sizeof( AVIStreamHeader ) ) < 0)
			return;
	...
	else if ( entry.type == make_fourcc( (char*) "indx" ) )
	{
		...
		if( lseek( fd, entry.offset, SEEK_SET ) == ( off_t ) - 1 )
			return;
		if(read( fd, &avi_super_index, sizeof( AVISuperIndex ) ) < 0)
			return;
	...
	else if ( entry.type == make_fourcc( (char*) "strf" ) )
	{
		if ( lastStreamType == make_fourcc( (char*) "auds" ) )
		{
			...
			if( lseek( fd, entry.offset, SEEK_SET ) == ( off_t ) - 1 )
				return;
			if(read( fd, &waveformatex, sizeof( WAVEFORMATEX ) ) < 0)
				return;
		...
		else if ( lastStreamType == make_fourcc( (char*) "vids" ) )
		{
			...
			if( lseek( fd, entry.offset, SEEK_SET ) == ( off_t ) - 1 )
				return;
			if(read( fd, &bitmapinfo, sizeof( BITMAPINFOHEADER ) ) < 0)
				return;
		...
		else if ( lastStreamType == make_fourcc( (char*) "iavs" ) )
		{
			...
			if( lseek( fd, entry.offset, SEEK_SET ) == ( off_t ) - 1 )
				return;
			if(read( fd, &dvinfo, sizeof( DVINFO ) ) < 0)
				return;
	...
	else if ( ( entry.type == make_fourcc( (char*) "ix00" ) ) || ( entry.type == make_fourcc( (char*) "ix01" ) ) )
	{
		...
		if( lseek( fd, entry.offset, SEEK_SET ) == ( off_t ) - 1 )
			return;
		if(read( fd, &avi_std_index, sizeof( AVIStdIndex ) ) < 0)
			return;
	...
	else if ( entry.type == make_fourcc( (char*) "idx1" ) )
	{
		...
		if( lseek( fd, entry.offset, SEEK_SET ) == ( off_t ) - 1 )
			return;
		if(read( fd, idx1, entry.length ) < 0)
			return;
	...
	else if ( entry.type == make_fourcc( (char*) "dmlh" ) )
	{
		...
		if( lseek( fd, entry.offset, SEEK_SET ) == ( off_t ) - 1 )
			return;
		if(read( fd, dmlh, entry.length ) < 0)
			return;
...
bool AVI1File::WriteFrame( const Frame &frame )
{
	...
	frame_chunk = AddDirectoryEntry( make_fourcc( (char*) "00__" ), 0, frame.GetFrameSize(), movi_list );
...
bool AVI2File::WriteFrame( const Frame &frame )
{
	...
	frame_chunk = AddDirectoryEntry( make_fourcc( (char*) "00dc" ), 0, frame.GetFrameSize(), movi_list );
	if ( ( index_type & AVI_LARGE_INDEX ) && ( streamHdr[ 0 ].dwLength % IX00_INDEX_SIZE ) == 0 )
	{
		GetDirectoryEntry( frame_chunk, type, name, length, offset, parent );
		ix[ 0 ] ->qwBaseOffset = offset - RIFF_HEADERSIZE;
