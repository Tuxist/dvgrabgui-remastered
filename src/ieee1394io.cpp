...
iec61883Reader::iec61883Reader( int p, int c, int bufSize,
	BusResetHandler resetHandler, BusResetHandlerData data ) :
		IEEE1394Reader( c, bufSize ), m_port( p ), m_resetHandler( resetHandler),
...
//		fail_neg( iec61883_dv_fb_start( m_iec61883dv, channel ) );
		iec61883_dv_fb_start( m_iec61883dv, channel );
		if(m_iec61883dv)
			success = true;
		else
			success = false;
...
char* AVC::ReadableTransportStatus( int phyID )
{
	char* c = avc1394_vcr_decode_status( AVC::TransportStatus( phyID ));
	if(c)
		return c;
	else
...
bool AVC::IsRecording( int phyID )
{
	bool val = false;
	pthread_mutex_lock( &avc_mutex );
	if ( avc_handle != NULL )
	{
		if ( phyID >= 0 )
			val = (bool) avc1394_vcr_is_recording( avc_handle, phyID );
	}
	pthread_mutex_unlock( &avc_mutex );
	return val;
}
