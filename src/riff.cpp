...
void RIFFFile::ParseChunk( int parent )
{
	...
	if ( type == make_fourcc( (char*) "LIST" ) )
	{
		typesize = -sizeof( type );
		if( lseek( fd, typesize, SEEK_CUR ) == ( off_t ) - 1 )
			return;
		ParseList( parent );
	}
...
void RIFFFile::ParseRIFF( void )
{
	...
	off_t filesize;
	off_t pos;
	int container = AddDirectoryEntry( make_fourcc( (char*) "FILE" ), make_fourcc( (char*) "FILE" ), 0, RIFF_NO_PARENT );
...
void RIFFFile::ReadChunk( int chunk_index, void *data )
{
	...
	if( lseek( fd, entry.offset, SEEK_SET ) == ( off_t ) - 1 )
		return;
	if( read( fd, data, entry.length ) < 0)
		return;
}
...
void RIFFFile::WriteChunk( int chunk_index, const void *data )
{
	...
	entry = GetDirectoryEntry( chunk_index );
	if( lseek( fd, entry.offset - RIFF_HEADERSIZE, SEEK_SET ) == ( off_t ) - 1 )
		return;
	if( write( fd, &entry.type, sizeof( entry.type ) ) < 0)
		return;
	DWORD length = entry.length;
	if( write( fd, &length, sizeof( length ) ) < 0)
		return;
	if( write( fd, data, entry.length ) < 0)
		return;
...
void RIFFFile::WriteRIFF( void )
{
	...
	for ( i = 1; i < count; ++i )
	{
		...
		if ( entry.written == false )
		{
			...
			if ( entry.name != 0 )
			{
				if( write( fd, &entry.name, sizeof( entry.name ) ) < 0)
					return;
			}
